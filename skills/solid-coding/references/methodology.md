...
Bound the scope explicitly every time: refactor mode never expands past the requested path/diff on its own initiative, and advisory mode never expands past the files actually touched in the current session. A reviewer's call-site trace may still *see* the same defect in files outside that scope; it records those as `out_of_scope_occurrences` on the in-scope finding they duplicate, rather than reporting them as findings of their own (see phase 5).
...
Dispatch `solid-verifier` (see `../../agents/solid-verifier.md`) once per finding — these can run in parallel with each other. Each call receives exactly one finding plus the code it refers to and the requested `scope`; the verifier re-derives the violation from the code itself rather than trusting the reviewer's summary, checks behavior preservation via its own call-site trace, and checks the proposed fix against `principle-tensions.md`. Its trace ranges wider than the scope by design, so it also returns `out_of_scope_occurrences`: every copy of the same defect outside the requested scope that it has re-read and confirmed — the reviewer's, if any, plus any its own grep turned up. This list, not the reviewer's, feeds the phase-5 section; a `REJECTED` finding returns it empty.
...
- **Out-of-scope occurrences:** the same defect as a confirmed or deferred finding, found by `solid-verifier` (or passed on from `solid-reviewer` and re-confirmed by it) in files outside the requested scope. One entry per occurrence: `path:line`, the principle, and the proposed fix (usually the in-scope finding's fix, applied at that site). These are reported for the user to act on, never acted on by this skill: phase 4 does not apply them without the user asking — a request to fix one is the user expanding scope, and only then does it go through phase 4 like any confirmed fix. Describe them only as reported. This skill has no mechanism to file an issue, queue a task, or schedule a follow-up, so never describe an occurrence as filed, queued, tracked, or fixed. If there are none, write "None" — an absent section reads the same as one nobody checked.
